...
void puzzle_4b() async {
  ...
  print('Puzzle 4b answer is: $answer');
}

final regex = RegExp(r'([a-zA-Z]{3}):(\S+)');

Map<String, dynamic> parseKeyValues(String line) => regex.allMatches(line).fold(<String, dynamic>{}, matchToKeyValue);
...
bool isHgtValid(String val) {
  ...
  if (cmMatches.isEmpty && inMatches.isEmpty) {
    return false;
  }

  return (cmMatches.isNotEmpty) ? cmBetween(cmMatches.first.group(1)) : inBetween(inMatches.first.group(1));
}
